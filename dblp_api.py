...
_LOCAL_DBLP_SYNC_LOCKFILE = os.environ.get("DBLP_SYNC_LOCKFILE_PATH", os.path.join(os.getcwd(), ".cache", "dblp.sync.lock"))
...
def ensure_local_dblp_dataset_fresh(max_age_hours: float = 24.0) -> None:
    ...
    with _DATASET_LOCK:
        ...
        os.makedirs(os.path.dirname(_LOCAL_DBLP_SYNC_LOCKFILE), exist_ok=True)
        try:
            lock_fd = os.open(_LOCAL_DBLP_SYNC_LOCKFILE, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
        except FileExistsError:
            logger.info("Another DBLP dataset sync is already running; skipping duplicate sync request")
            return
        ...
        try:
            ...
            if lock_fd is not None:
                os.close(lock_fd)
            try:
                os.remove(_LOCAL_DBLP_SYNC_LOCKFILE)
            except OSError:
                pass
